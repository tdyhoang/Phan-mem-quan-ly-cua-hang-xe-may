        ...
        private void SaveToDatabase(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    cmd.ExecuteNonQuery();
                    trans.Commit();
                    cmd = new($"Set Dateformat dmy\nInsert into LichSuHoatDong values(newid(), '{PageChinh.getMa}', '{DateTime.Now:dd-MM-yyyy HH:mm:ss}', N'chỉnh sửa database nhà cung cấp')", con);
                    cmd.ExecuteNonQuery();
        ...
        private void Export(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                using (ExcelPackage p = new())
                {
                    ...
                    ws.Cells[1, 1, 1, grdSupplier.Columns.Count].Merge = true;
                    // in đậm
                    ws.Cells[1, 1, 1, grdSupplier.Columns.Count].Style.Font.Bold = true;
                    // căn giữa
                    ws.Cells[1, 1, 1, grdSupplier.Columns.Count].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

                    int colIndex = 1;
                    int rowIndex = 2;

                    //tạo các header từ column header đã tạo từ bên trên
                    foreach (var item in grdSupplier.Columns)
                    {
                    ...
                    foreach (var ncc in SupplierList.Where(ncc => grdSupplier.Items.PassesFilter(ncc)))
                    {
                ...
                using SqlConnection con = new(Settings.Default.ConnectionString);
                con.Open();
                SqlCommand cmd = new($"Set Dateformat dmy\nInsert into LichSuHoatDong values(newid(), '{PageChinh.getMa}', '{DateTime.Now:dd-MM-yyyy HH:mm:ss}', N'xuất excel nhà cung cấp')", con);
                cmd.ExecuteNonQuery();
