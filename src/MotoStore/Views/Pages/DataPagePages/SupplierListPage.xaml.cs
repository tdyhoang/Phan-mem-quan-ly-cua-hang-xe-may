                ...
                try
                {
                    ...
                    foreach (var obj in grdSupplier.Items)
                    {
                        ...
                        if (string.IsNullOrEmpty(ncc.MaNcc))
                            cmd.CommandText += $"\nInsert into KhachHang values(N'{ncc.TenNcc}', '{ncc.Sdt}', N'{ncc.Email}', N'{ncc.DiaChi}', 0)";

                        // Cập nhật
                        else
                            cmd.CommandText += $"\nUpdate KhachHang Set TenNcc = N'{ncc.TenNcc}', DiaChi = N'{ncc.DiaChi}', Sdt = '{ncc.Sdt}', Email = N'{ncc.Email}' Where MaNcc = '{ncc.MaNcc}';";
                    }
                    cmd.ExecuteNonQuery();
                    trans.Commit();
